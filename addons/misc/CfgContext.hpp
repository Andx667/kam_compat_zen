class zen_context_menu_actions {
    class KAM_MENU_CATEGORY {
        class GVAR(enableAiDeathPrevention) {
            ...
            condition = QUOTE(GVAR(enableZenActions) && {KAM_HOVERED_UNIT} && {alive _hoveredEntity} && {!isPlayer _hoveredEntity} && {!(_hoveredEntity getVariable [ARR_2('KATGVAR(misc,PreventInstantAIDeath)',false)])});
